...
def transmit_hackrf(iq_data, frequency=FREQUENCY, sample_rate=SAMPLE_RATE):
    try:
        ...
        print(f"Ошибка при работе с HackRF: {e}")
        if hackrf:
            hackrf.close()
...
# Убедитесь, что HackRF подключен и распознан
if __name__ == "__main__":
    # Проверка наличия HackRF
    try:
        os.system("hackrf_info")
        print("HackRF обнаружен, начинаем передачу.")
        send_packet_repeatedly(packet, repeats=4, pause_duration=PAUSE_BETWEEN_REPEATS)
    except Exception as e:
        print(f"Ошибка: HackRF не найден или не настроен. Убедитесь, что драйверы и библиотeki установлены. Ошибка: {e}")
